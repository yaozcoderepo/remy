...
for entry in entries:
    ...
    if replots:
        ...
        for replot in replots:
            ...
            try:
                replot_argsfile = open(os.path.join(
                    dirname, "replots", replot_basename, "args.json"))
                replot_args = json.load(replot_argsfile)["args"]
                replot_argsfile.close()
                replot_remyccs = " ".join(
                    [os.path.basename(r) for r in replot_args["remycc"]])
                replot_npoints = replot_args["num_points"]
            except (IOError, KeyError):
                replots_str += " {basename}<error>".format(
                    basename=replot_basename)
            else:
                if replot_remyccs:
                    replots_str += " {basename}[{remyccs}/{npoints}]".format(basename=replot_basename,
                                                                             remyccs=replot_remyccs, npoints=replot_npoints)
                else:
                    replots_str += " {basename}[]".format(
                        basename=replot_basename)
    else:
        replots_str = ""

    plotsdirname = os.path.join(dirname, "plots")
    noplots = "[no plots] " if os.path.isdir(plotsdirname) and len(
        os.listdir(plotsdirname)) == 0 else ""
