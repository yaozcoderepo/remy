                              ...
                              const int largest_ack)
{
    for (const auto &x : packets)
    {
        ...
        else
        {
            ...
            _slow_rec_send_ewma = (1 - slow_alpha) * _rec_send_ewma + alpha * (x.tick_sent - _last_tick_sent);
...
{
    char tmp[256];
    snprintf(tmp, 256, "sewma=%f, rewma=%f, rttr=%f, slowrewma=%f, rttd=%f, qdelay=%f, slowsewma=%f,", _rec_send_ewma, _rec_rec_ewma, _rtt_ratio, _slow_rec_rec_ewma, _rtt_diff, _queueing_delay, _slow_rec_send_ewma);
...
string Memory::str(unsigned int num) const
{
    ...
    case 6:
        ...
        snprintf(tmp, 50, "slowsewma=%f ", _slow_rec_send_ewma);
        break;
    }
    return tmp;
}
...
RemyBuffers::Memory Memory::DNA(void) const
{
    ...
    ret.set_slow_rec_send_ewma(_slow_rec_send_ewma);
    return ret;
}
...
#define get_val_or_default(protobuf, field, limit)               \
    ((protobuf).has_##field() ? (protobuf).field() : (limit) ? 0 \
                                                             : 163840)
...
Memory::Memory(const bool is_lower_limit, const RemyBuffers::Memory &dna)
    : _rec_send_ewma(get_val_or_default(dna, rec_send_ewma, is_lower_limit)),
      ...
      _slow_rec_send_ewma(get_val_or_default(dna, slow_rec_send_ewma, is_lower_limit)),
      _last_tick_sent(0),
...
size_t hash_value(const Memory &mem)
{
    ...
    boost::hash_combine(seed, mem._slow_rec_send_ewma);
